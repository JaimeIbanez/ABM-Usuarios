...
Mongo.connect('mongodb://localhost:27017/abm', function(err, db) {
	...
	router.get('/:id', function(req, res) {
		return users.findOne({ id: req.params.id }, function (err, post) {
	    	if (err) { throw(err); }

	    	return res.render('users', {users: post.users, url: post.URL /*, other data you need... */});
  		});

		//res.render('adminMod/:id', { css: './stylesheets/dashboard.css'});
	});
